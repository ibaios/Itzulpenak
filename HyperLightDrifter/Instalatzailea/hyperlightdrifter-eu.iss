...
#define MyAppIcon "hyperlightdrifter-eu.ico"
#define MyAppImage "hyperlightdrifter-eu-instalazioa.bmp"

[Setup]
; NOTE: The value of AppId uniquely identifies this application. Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
AppId={{6C88D3A5-114B-4F73-9A38-70FCE06CBE1B}
AppName={#MyAppName}
...
begin
  ...
  // Steam
  GamePath := FindGamePathInList(GetSteamLibraryPaths(), '\steamapps\common\' + 'HyperLightDrifter');  
  
  if GamePath = '' then
  begin
    // GOG Galaxy
    GamePath := FindGamePathInList(GetGOGPaths(), 'HyperLightDrifter');
    ...
    begin
      ...
      if DirExists(ExpandConstant('{autopf}') + 'HyperLightDrifter') then
        GamePath := ExpandConstant('{autopf}') + 'HyperLightDrifter';
...
begin
  GameDataDirPage := CreateInputDirPage(wpSelectDir,
    ...
    '',
    'Hautatu "Hyper Light Drifter" jokoaren datuen karpeta non dagoen.'
    + #13#10#13#10 + 'Instalazioa automatikoki antzematen saiatuko da, baina ez badu lortzen, zuk bete beharko duzu.'
    + #13#10#13#10 + 'Aukeratu jokoaren karpeta nagusia (''HyperLightDrifter'' izenekoa).',
    False,'');
