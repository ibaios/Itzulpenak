...
[Run]
Filename: "{app}\uabea\UABEAvalonia.exe"; Parameters: "applyemip ""{app}\{code:GetSelectedEmip}"" ""{code:GetSelectedGameDataDir}"""
...
begin
  ...
  // Steam
  GamePath := FindGamePathInList(GetSteamLibraryPaths(), '\steamapps\common\' + 'Hollow Knight\hollow_knight_Data');  
  
  if GamePath = '' then
  begin
    // GOG Galaxy
    GamePath := FindGamePathInList(GetGOGPaths(), 'Hollow Knight\hollow_knight_Data');
    ...
    begin
      ...
      if DirExists(ExpandConstant('{autopf}') + 'Hollow Knight\hollow_knight_Data') then
        GamePath := ExpandConstant('{autopf}') + 'Hollow Knight\hollow_knight_Data';
...
begin
  GameDataDirPage := CreateInputDirPage(wpSelectDir,
    ...
    '',
    'Hautatu "Hollow Knight" jokoaren datuen karpeta non dagoen.'
    + #13#10#13#10 + 'Instalazioa automatikoki antzematen saiatuko da, baina ez badu lortzen, zuk bete beharko duzu.'
    + #13#10#13#10 + 'Aurkitu jokoaren karpeta nagusia (''Hollow Knight''), eta aukeratu honen barruan dagoen ''hollow_knight_Data'' izeneko karpeta.',
    False,'');
