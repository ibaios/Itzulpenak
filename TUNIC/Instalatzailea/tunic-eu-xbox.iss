...
#define MyAppImage "tunic-eu-instalazioa.bmp"

[Setup]
; NOTE: The value of AppId uniquely identifies this application. Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
AppId={{D9A69E65-82D5-49E1-9935-0DF69A29E358}
AppName={#MyAppName}
...
OutputBaseFilename={#MyAppFilesystemName} XBOX
Compression=lzma
...
begin
  ...
  // Steam
  GamePath := FindGamePathInList(GetSteamLibraryPaths(), '\steamapps\common\' + 'TUNIC\Tunic_Data');  
  
  if GamePath = '' then
  begin
    // GOG Galaxy
    GamePath := FindGamePathInList(GetGOGPaths(), 'TUNIC\Tunic_Data');
    ...
    begin
      ...
      if DirExists(ExpandConstant('{autopf}') + 'TUNIC\Tunic_Data') then
        GamePath := ExpandConstant('{autopf}') + 'TUNIC\Tunic_Data';
...
begin
  GameDataDirPage := CreateInputDirPage(wpSelectDir,
    ...
    '',
    'Hautatu "TUNIC" jokoaren datuen karpeta non dagoen.'
    + #13#10#13#10 + 'Instalazioa automatikoki antzematen saiatuko da, baina ez badu lortzen, zuk bete beharko duzu.'
    + #13#10#13#10 + 'Aurkitu jokoaren karpeta nagusia (''TUNIC''), eta aukeratu honen barruan dagoen ''Tunic_Data'' izeneko karpeta.',
    False,'');
